...
def backtest(df, UPPER, LOWER, grid_mode,
             NUM=200, TX_FEE=0.0005, INVEST=1000,
             show_tx=True):
    ...
    for i in range(len(grids)):
        ...
        if grids[i+1] <= cur_price:
            ...
            status[i] = {'buy_price': grids[i], 'sell_price': grids[i+1], 'status': 'cash',
                        'bought_at': -1, 'holding': 0}
        else:
            # current price < grid sell price
            # this means: buy, wait to sell.
            status[i] = {'buy_price': grids[i], 'sell_price': grids[i+1], 'status': 'coin',
                        'bought_at': cur_price, 'holding': each_grid_coin}
    ...
    for minute_idx, row in df.iterrows():
        ...
        cur_price = df.close.iloc[minute_idx-1]  # previous closing price as opening price
        ...
        for j, grid in enumerate(grids):
            if grid <= cur_price:
            # if grid <= row.close:
                if j==NUM-1: continue  # top grid: only sell, no buy
                buy.append( j )
            elif grid >= cur_price:
            # elif grid > row.close:
                if j==0: continue  # bottom grid: no sell, only buy
                sell.append( j )
        ...
        if cnt > 0:
            ...
            for tx_i in tx_idx:
                ...
                if tx_i in sell:
                    # sell coin, get USDT
                    if show_tx: print(f'{row.timestamp}, {tx_i} SOLD, ${round(grids[tx_i],3)}')
                    
                    if status[tx_i-1]['status'] == 'cash' and first_tx_executed:
                        raise NoCoinToSell
                    day_sell_tx_cnt += 1
                    cash += (status[tx_i-1]['holding']*status[tx_i-1]['sell_price'])*(1-TX_FEE)
                    daily_grid_profit += (status[tx_i-1]['holding'] * ( (1-TX_FEE)*status[tx_i-1]['sell_price'] - status[tx_i-1]['bought_at']*(1+TX_FEE) ))
                    ...
                    # buy coin, get coin
                    if show_tx: print(f'{row.timestamp}, {tx_i} BOUGHT, ${round(grids[tx_i],3)}')
        ...
        # print(minute_idx+1)
        if minute_idx==len(df)-1 or df.timestamp.iloc[minute_idx+1].date() != prev_date:
    ...
    asset_evaluation(daily_sell_tx_cnts, status, cash, sum(daily_grid_profits), df.close.iloc[-1], INVEST=INVEST, NUM=NUM)

    return daily_sell_tx_cnts, cash, daily_grid_profits

if __name__=="__main__":
    df = get_data_since("BTC", datetime.datetime(2022, 11, 1, 0, 0), datetime.datetime(2022, 11, 2, 12, 30) )
